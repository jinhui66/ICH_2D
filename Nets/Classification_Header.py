...
"""
    Here, we use 1D DenseNet-121 as Classification Header
    You can use both pre-trained 1D DenseNet-121 or 1D DenseNet-121
    or you can use any other classifier as your Classification Header
    according to different datasets and your needs.
"""



def get_pretrained_Classification_Header():
    """
        If you want to use pre-trained 1D DenseNet, please download the Nets
        and place it in the specified directory location beforehand
        The Net can be downloaded at https://drive.google.com/file/d/0B8ReS-sYUS-HWFViYlVlZk9sdHc/view?resourcekey=0-ZO-PrGpkNxjNGaoxFBiddg
    """
    model = torch.load("trained_net/pretrained/densenet-121.t7")

    return model

def get_Classification_Header():
    model = DenseNet(layer_num=(6, 12, 24, 16), growth_rate=32, in_channels=1, classes=2)
    return model

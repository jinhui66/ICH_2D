...
import math
from Nets.Vision_Encoder import conv1x1,conv3x3
...
class Multi_Head_Self_Attention_Fusion(nn.Module):
    def __init__(self, num_attention_heads, input_size, hidden_size, hidden_dropout_prob, output_size):
        ...
        self.conv = conv3x3(hidden_size, output_size)
    ...
    def forward(self, input_tensor):
        ...
        context_layer = context_layer.reshape(-1, 2).contiguous()
...
class Cross_Modal_Attention_Fusion(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, planes, middle_size ,size):
        ...
        self.text_query = conv1x1(planes, self.expansion)
        self.text_key = conv1x1(planes, self.expansion)
        self.text_value = conv1x1(planes, self.expansion)

        self.vision_query = conv1x1(planes, self.expansion)
        self.vision_key = conv1x1(planes, self.expansion)
        self.vision_value = conv1x1(planes, self.expansion)

        self.text_fc2 = nn.Linear(hidden_size, middle_size)
        self.vision_fc2 = nn.Linear(hidden_size, middle_size)

        self.softpool_x1 = SoftPool2d(kernel_size=(self.C_hat,  2), stride=(2, 2, 2))
        self.softpool_x2 = SoftPool2d(kernel_size=(self.C_hat,  2), stride=(2, 2, 2))
        self.softpool_y1 = SoftPool2d(kernel_size=(self.C_hat,  2), stride=(2, 2, 2))
        self.softpool_y2 = SoftPool2d(kernel_size=(self.C_hat,  2), stride=(2, 2, 2))
    ...
    def forward(self, x, y):
        ...
        query_t = query_t.reshape(-1, 2)
        query_v = query_v.reshape(-1, 2)
